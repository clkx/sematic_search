...
from generate_embedding import hf_embedding, audio_embedding
from search import keyword_search as kwdsearch, semantic_search_by_prompt as ssearch_prompt, semantic_search_by_audio as ssearch_audio
...
@app.post("/items/", response_model=GeneratedMusic)
...
@app.get("/items/id/{id}", response_model=GeneratedMusic)
...
@app.get("/items/prompt/semantic/{prompt}")
...
@app.put("/items/{id}", response_model=GeneratedMusic)
...
@app.delete("/items/{id}", response_model=GeneratedMusic)
...
async def audio_search(file: UploadFile = File(...), limit: Optional[int] = 10):
    ...
    try:
        items = ssearch_audio(temp_file_path, limit)  # Ensure this can work with async or consider making the call synchronous if it must be
        return list(items)
    finally:
